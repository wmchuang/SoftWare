...
namespace Software.Repository.Base
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class, new()
    {
        ...
        {
            return await Task.Run(() => db.Queryable<TEntity>().Single(expression));
        }
        /// <summary>
        /// 功能描述:根据ID查询一条数据
        /// </summary>
        ...
        {
            return await Task.Run(() => db.Queryable<TEntity>().WhereIF(whereExpression != null, whereExpression).Any());
        }
        ...
        public List<TEntity> QueryPage(ref int totalCount, Expression<Func<TEntity, bool>> whereExpression,
        int intPageIndex = 0, int intPageSize = 20, string strOrderByFileds = null)
        {
        ...
        public async Task<dynamic> SqlQueryDynamic(string spName, object whereObj = null)
        {
            return await Task.Run(() =>
               db.Ado.UseStoredProcedure<dynamic>(() =>
               {
                   return db.Ado.SqlQueryDynamic(spName, whereObj);
               })
         );
        }
